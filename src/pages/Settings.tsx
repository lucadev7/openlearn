...
import { Check, Database, Eye, EyeOff, Palette, Sparkles, Trash2 } from "lucide-react";
      ...
      {tab === "data" && (
        <GlassCard className="space-y-3 text-sm">
          ...
          <p className="pt-2 text-muted">
            Alle Lern- und Fortschrittsdaten bleiben lokal auf deinem Gerät (SQLite). API-Keys
            liegen ausschließlich im Windows Credential Manager. Backup/Restore und
            Integritätsprüfung folgen in Phase 7.
          </p>
          <p className="text-xs text-muted/80">
            Hinweis: Release-Builds sind aktuell nicht signiert — Windows SmartScreen kann beim
            ersten Start eine Warnung zeigen („Weitere Informationen → Trotzdem ausführen").
          </p>
        </GlassCard>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <GlassCard className="space-y-4">
        ...
        <p className="text-xs text-muted/80">
          KI-Verbindungstest und die KI-Funktionen (Tutor, Generierung, Bewertung, Lernpfade,
          mündliche Prüfung) werden in Phase 2–4 aktiviert. Keys kannst du aber bereits hinterlegen.
        </p>
      </GlassCard>
    </div>
  );
